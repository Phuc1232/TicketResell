...
class TransactionRepository(ITransactionRepository):
    ...
    def get_by_id(self, transaction_id: int) -> Optional[Transaction]:
        model = session.query(TransactionModel).filter(TransactionModel.TransactionID == transaction_id).first()
        return self._to_domain(model) if model else None
    
    def get_by_ticket_id(self, ticket_id: int) -> List[Transaction]:
        models = session.query(TransactionModel).filter(TransactionModel.TicketID == ticket_id).all()
        return [self._to_domain(model) for model in models]
    
    def get_by_user_id(self, user_id: int) -> List[Transaction]:
        models = session.query(TransactionModel).filter(
            (TransactionModel.BuyerID == user_id) | (TransactionModel.SellerID == user_id)
        ).all()
        return [self._to_domain(model) for model in models]
    
    def update(self, transaction: Transaction) -> Transaction:
        model = session.query(TransactionModel).filter(TransactionModel.TransactionID == transaction.TransactionID).first()
        if model:
